...
public class NuageToxique extends Objet implements Combattant, Deplacable {
    ...
    /**
     * Combat pour le nuage Si le joueur passe dessous (donc distance néagtive), il
     * attaque de la même façon qu'un guerrier en prenant en compte les points
     * 
     * @param c
     */
    ...
    public void combattre(Creature c) {
        ...
        for (int i = 0; i < listeCase.size(); i++) {
            ...
            if ((this.getListeCase().get(i).getX() == c.getPos().getX())
                    & (this.getListeCase().get(i).getY() == c.getPos().getY())) {
                System.out.println("Le nuage vous fait de l'oeil !");
